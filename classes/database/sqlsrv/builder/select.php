...
class Database_Sqlsrv_Builder_Select extends \Database_Query_Builder_Where
{
    ...
    public function compile($db = null)
    {
        ...
        // set order for pagination if not set order by
        elseif (($this->_order_by !== NULL || $this->_limit !== NULL) && $this->_order_by == NULL)
        {
            $query .= ' ORDER BY (SELECT 0) ';
        }

        // if set limit or offset
        if ($this->_offset !== NULL || $this->_limit !== NULL)
        {
            // set offset to '0' for get_one()
            if ($this->_offset == 0)
            {
                $this->_offset = '0';
            }
            // limit and offset is set
